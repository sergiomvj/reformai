...
import { HardHat, Plus, Hammer, Trash, CheckCircle, Sparkles, Download, FileSpreadsheet, Edit3, X, TrendingUp, Video, Camera, LogIn, LogOut, History, Building } from './components/Icons';
...
const App: React.FC = () => {
  ...
  if (!session) return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[3rem] p-10 shadow-2xl animate-in zoom-in duration-300">
        <div className="text-center mb-8">
          <div className="bg-amber-500 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 text-slate-900 shadow-xl">
        ...
        <form onSubmit={isRegistering ? handleSignUp : handleLogin} className="space-y-4">
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">{translations[lang].email}</label>
            <input
              type="email" required value={authEmail} onChange={e => setAuthEmail(e.target.value)}
              className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 focus:border-amber-500 outline-none transition-all font-bold text-slate-800 bg-slate-50"
            />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">{translations[lang].password}</label>
            <input
          ...
          <button
            ...
            className="w-full bg-slate-900 text-white font-black py-5 rounded-2xl hover:bg-slate-800 transition-all shadow-xl uppercase tracking-widest mt-4 flex items-center justify-center"
          >
        ...
        <button
          ...
          className="w-full mt-6 text-[10px] font-black text-slate-400 uppercase hover:text-amber-500 transition-colors"
        >
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32">
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-slate-800 uppercase">{translations[lang].newProject}</h3>
              <button onClick={() => setShowProjectModal(false)}><X className="text-slate-300" /></button>
            </div>
            <div className="space-y-4">
              <input
                type="text" placeholder={translations[lang].projectName} value={newProjectName} onChange={e => setNewProjectName(e.target.value)}
                className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 focus:border-amber-500 outline-none font-bold"
              />
              <button onClick={createProject} className="w-full bg-amber-500 text-slate-900 font-black py-4 rounded-2xl hover:bg-amber-400 transition-all uppercase tracking-widest">
      ...
      <div className="sticky top-0 z-50 shadow-lg">
        ...
        <div className="bg-amber-500 py-3 px-4 border-b border-amber-600">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-4 overflow-x-auto no-scrollbar scroll-smooth">
              ...
              {projects.map(p => (
                <button
                  ...
                  className={`flex-shrink-0 px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${currentProject?.id === p.id ? 'bg-slate-900 text-white shadow-xl scale-105' : 'bg-amber-400 text-amber-900 hover:bg-amber-300'}`}
                >
                  {p.name}
                </button>
              ))}
            ...
            <div className="hidden md:flex gap-3 ml-4">
              {['en', 'sp', 'pt-br'].map(l => (
                <button
                  ...
                  className={`text-[10px] font-black uppercase transition-colors ${lang === l ? 'text-slate-900' : 'text-amber-800 hover:text-slate-900'}`}
                >
                  {l}
                </button>
              ))}
          ...
          {showDashboard && (
            <section className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl border-b-[10px] border-amber-500 animate-in zoom-in duration-300">
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h2 className="text-amber-500 font-black uppercase tracking-[0.2em] text-[10px] mb-1">{translations[lang].consolidationStatus}</h2>
                  <p className="text-3xl font-black uppercase">{translations[lang].panelTitle}<span className="text-amber-500">AI</span></p>
                  <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest opacity-60">{currentProject.name}</p>
                </div>
                <div className="flex items-center gap-4">
                  <button
                  ...
                  >
                    <History className="w-4 h-4" /> {translations[lang].workLog}
                  </button>
                  <div className="text-right">
                    <p className="text-5xl font-black text-amber-500">{stats.totalProgress}%</p>
                    <p className="text-[9px] text-slate-500 uppercase font-black">{translations[lang].totalExecution}</p>
                  </div>
                </div>
              </div>
              ...
              {showHistory ? (
                <div className="animate-in slide-in-from-top-4 duration-300">
                  <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-white/10 pb-2 mb-4">{translations[lang].historyRecent}</h3>
                  <div className="max-h-80 overflow-y-auto pr-2 custom-scrollbar space-y-3">
                    {history.length > 0 ? history.map(entry => (
                      <div key={entry.id} className="bg-white/5 border border-white/5 p-4 rounded-2xl flex items-start gap-4 hover:bg-white/10 transition-all">
                        <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${entry.type === 'completion' ? 'bg-green-500' :
                      ...
                      <p className="text-[10px] text-slate-600 italic py-4">{translations[lang].noHistory}</p>
                    )}
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-6">
                    <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-white/10 pb-2">{translations[lang].progressByPriority}</h3>
                  ...
                  <div className="space-y-6">
                    <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-white/10 pb-2">{translations[lang].progressByRoom}</h3>
                    <div className="max-h-56 overflow-y-auto pr-3 space-y-4 custom-scrollbar">
                      ...
                      {Object.keys(stats.byRoom).length === 0 && <p className="text-[10px] text-slate-600 italic">{translations[lang].noActiveRoom}</p>}
                    </div>
                  </div>
                </div>
              )}
          ...
          {!result && (
            <section className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-200 space-y-8">
              <div className="flex justify-between items-center">
                <div className="space-y-1">
                  <h2 className="text-xl font-black text-slate-800 uppercase">{translations[lang].newTechnicalRecord}</h2>
                  <p className="text-slate-500 text-xs">{translations[lang].selectCategory}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">{translations[lang].locationRoom}</label>
                  <input
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">{translations[lang].criticalPriority}</label>
                  <div className="flex gap-2">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">{translations[lang].workCategories}</label>
              ...
              {selectedCat && (
                <div className="space-y-5 p-6 bg-amber-50 rounded-[2rem] border-2 border-amber-100 animate-in slide-in-from-top-4 duration-300">
                  <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest">{translations[lang].specializedTasks}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">{translations[lang].mediaReference}</label>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <span className="text-[10px] font-black uppercase text-center">{translations[lang].video}</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="text-[10px] font-black uppercase text-center">{photoFiles.length > 0 ? `${photoFiles.length} ${translations[lang].photos}` : translations[lang].photos}</span>
                    </button>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">{translations[lang].masterNotes}</label>
                  <textarea
              ...
              >
                <Plus className="w-6 h-6" /> {translations[lang].incorporateTask}
              </button>
            </section>
          )}
          ...
          {tasks.length > 0 && !result && (
            <section className="space-y-8">
              <div className="bg-slate-900 p-8 rounded-[3rem] text-white shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6 border-l-8 border-amber-500">
                <div>
                  <h3 className="text-[10px] font-black uppercase text-amber-500 mb-2 tracking-widest">{translations[lang].scheduleEngineering}</h3>
                  <p className="text-xl font-heading uppercase">{translations[lang].chooseActionPlan}</p>
                </div>
                <div className="flex flex-wrap gap-2 justify-center">
                  <button onClick={() => handleOptimize('fastest')} className="bg-white/10 hover:bg-amber-500 hover:text-slate-900 px-5 py-3 rounded-2xl text-[9px] font-black uppercase transition-all flex items-center gap-2">
                    <Sparkles className="w-4 h-4" /> {translations[lang].fastestExecution}
                  </button>
                  <button onClick={() => handleOptimize('priority')} className="bg-white/10 hover:bg-amber-500 hover:text-slate-900 px-5 py-3 rounded-2xl text-[9px] font-black uppercase transition-all flex items-center gap-2">
                    <TrendingUp className="w-4 h-4" /> {translations[lang].byPriority}
                  </button>
                  <button onClick={() => handleOptimize('room')} className="bg-white/10 hover:bg-amber-500 hover:text-slate-900 px-5 py-3 rounded-2xl text-[9px] font-black uppercase transition-all flex items-center gap-2">
                    <HardHat className="w-4 h-4" /> {translations[lang].byRoom}
                  </button>
                </div>
              </div>
                      ...
                      <div className="p-8">
                        <div className="flex justify-between items-start mb-6">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1">{translations[lang].completed}</p>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-50 p-6 rounded-[2rem] space-y-4">
                          <div className="flex justify-between items-center">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{translations[lang].activityBreakdown}</p>
                            <span className="text-[9px] font-bold text-slate-300">{translations[lang].executivePhase}</span>
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {task.subTasks.map(st => (
                              <button
                                key={st.id} onClick={() => toggleSubTask(task.id, st.id)}
                                className={`flex items-center gap-4 p-4 rounded-2xl text-[10px] font-bold text-left transition-all ${st.completed ? 'bg-green-100 text-green-700 border-green-200' : 'bg-white border border-slate-200 text-slate-600 shadow-sm hover:border-amber-300'}`}
                              >
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-100 flex justify-end">
                          <button onClick={() => deleteTask(task.id)} className="text-[9px] font-black text-slate-300 hover:text-red-500 uppercase tracking-widest flex items-center gap-2">
                            <Trash className="w-3.5 h-3.5" /> {translations[lang].deleteRecord}
                          </button>
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-12 animate-in slide-in-from-bottom-6 duration-700">
              <div className="bg-slate-900 text-white p-12 rounded-[4rem] border-b-[15px] border-amber-500 shadow-2xl relative overflow-hidden">
                <div className="flex justify-between items-start mb-10">
                  <h2 className="text-4xl font-heading uppercase tracking-widest flex items-center gap-5"><Sparkles className="text-amber-500 w-10 h-10" /> {translations[lang].masterPlan}</h2>
                  <button onClick={() => setResult(null)} className="p-4 bg-white/10 rounded-full hover:bg-white/20 transition-all"><X /></button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2">{translations[lang].executionWindow}</p>
                    <p className="text-6xl font-black text-amber-500">{result.totalEstimatedDays} {translations[lang].days}</p>
                  </div>
                  <div className="bg-white/5 p-6 rounded-3xl border border-white/10">
                    <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-3">{translations[lang].directive}:</p>
                ...
                {result.phases.map(phase => (
                  <div key={phase.order} className="bg-white rounded-[3rem] shadow-xl border border-slate-200 overflow-hidden">
                    <div className="bg-slate-900 text-white px-10 py-8 flex justify-between items-center border-b-4 border-amber-500">
                      ...
                      <div className="bg-amber-500 text-slate-900 px-6 py-2 rounded-full text-[12px] font-black uppercase tracking-widest shadow-2xl">{translations[lang].step} {phase.order}</div>
                    </div>
                      ...
                      {phase.tasks.map(t => (
                        <div key={t.id} className="relative pl-14 border-l-4 border-slate-100 pb-10 last:pb-0 group">
                          ...
                          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                            ...
                            <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">{translations[lang].order} {t.sequenceOrder}</span>
                          </div>
                          {t.reasoning && (
                            <div className="mt-5 p-5 bg-slate-50 rounded-3xl border border-slate-100 flex gap-4">
                              ...
                              <p className="text-[11px] text-slate-500 leading-relaxed italic font-medium">{translations[lang].logisticsAI}: {t.reasoning}</p>
                            </div>
                          )}
                        </div>
                      ))}
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 py-5 px-8 flex justify-between items-center z-[40] shadow-2xl">
        <div className="flex gap-8">
          <div className="flex flex-col">
            <span className="text-[11px] font-black text-slate-900 uppercase tracking-widest">{tasks.length} {translations[lang].activities}</span>
          ...
          <div className="flex flex-col">
            <span className="text-[11px] font-black text-amber-500 uppercase tracking-widest">{stats.totalProgress}% {translations[lang].work}</span>
        ...
        <div className="text-right flex flex-col items-end">
          ...
          <span className="text-[7px] font-bold text-slate-400 uppercase bg-slate-50 px-2 py-0.5 rounded border border-slate-100">Session: {session?.user?.email}</span>
        </div>
      </footer>
    </div>
  );
